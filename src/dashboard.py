...
st.set_page_config(
    page_title="CriPri Crime Analysis Dashboard",
    page_icon="🔴",
    layout="wide",
...
SEVERITY_WEIGHTS = {
    "HOMICIDE": 10, "SEXUAL ASSAULT": 9,"KIDNAPPING": 8, "FIREARM OFFENSE": 8,"ARSON": 7, "ASSAULT": 7, "ROBBERY": 7,
    "EXTORTION": 6, "DOMESTIC VIOLENCE": 6,"ILLEGAL POSSESSION": 5, "DRUG OFFENSE": 5, "BURGLARY": 5,"FRAUD": 4, "IDENTITY THEFT": 4, "CYBERCRIME": 4,
    "VEHICLE - STOLEN": 3, "COUNTERFEITING": 3, "VANDALISM": 3, "SHOPLIFTING": 2, "TRAFFIC VIOLATION": 2, "PUBLIC INTOXICATION": 1,
}
...
def apply_filters(df, city, crime, bucket, season, risk_cat, year_range):
    filtered = df.copy()
    if city != "All Cities":
...
def render_sidebar(df, risk):
    ...
    <div style='text-align:center; padding:10px 0'>
        ...
        <p style='color:#aaa; margin:0; font-size:12px'>Crime Intelligence Platform</p>
    </div>
    """, unsafe_allow_html=True)

    st.sidebar.markdown("### Risk Leaderboard")
    ...
    for i, (_, row) in enumerate(top10.iterrows(), 1):
        ...
        st.sidebar.markdown(
            ...
            f'<b>{i}. {row["City"]}</b>'
            f'<span style="float:right; color:{color}">{row["Risk_Index"]:.1f}</span>'
            f'</div>'
    ...
    crimes = ["All Crime Types"] + sorted(df["Crime_Description"].unique().tolist())
    ...
    st.sidebar.markdown("*Apply filters then download from Export tab*")
    st.sidebar.markdown(
        '<div style="text-align:center; color:#666; font-size:11px">'
        'Built with Python & Streamlit<br>'
        '<a href="https://github.com/sarthak29-hub/CriPri" style="color:#4fc3f7">'
...
def render_kpis(filtered, df):
    total = len(filtered)
    top_city = filtered["City"].value_counts().index[0] if total > 0 else "N/A"
    dominant = filtered["Crime_Description"].value_counts().index[0] if total > 0 else "N/A"
    avg_risk = filtered["Risk_Index"].mean() if total > 0 else 0
    closure = round((filtered["Case_Closed"] == "Yes").sum() / total * 100, 1) if total > 0 else 0
    violent_pct = round(filtered["Is_Violent"].mean() * 100, 1) if total > 0 else 0
    night_pct = round((filtered["Time_Bucket"] == "Night").sum() / total * 100, 1) if total > 0 else 0
    pct_total = round(total / len(df) * 100, 1)
    dom_short = dominant[:13] + "..." if len(dominant) > 13 else dominant

    c1, c2, c3, c4, c5, c6 = st.columns(6)
    c1.metric("Total Crimes",f"{total:,}",delta=f"{pct_total}% of dataset")
    c2.metric("Top Crime City",top_city)
    c3.metric("Dominant Crime",dom_short)
    c4.metric("Avg Risk Index",f"{avg_risk:.1f}",delta="/ 100")
    c5.metric("Case Closure",f"{closure}%")
    c6.metric("Violent Crime %",f"{violent_pct}%",delta=f"Night: {night_pct}%")

def render_folium(html_string, height=580):
    ...
    if map_type == "Crime Heatmap":
        ...
        if os.path.exists(HEATMAP_PATH):
            with open(HEATMAP_PATH, "r", encoding="utf-8") as f:
                render_folium(f.read(), height=580)
        else:
            st.error("Run hotspot_analysis.py first.")

    elif map_type == "Risk Zone Map":
        st.caption("Color coded risk index per city 0 to 100")
        if os.path.exists(RISK_MAP_PATH):
            with open(RISK_MAP_PATH, "r", encoding="utf-8") as f:
                render_folium(f.read(), height=580)
        else:
            st.error("Run risk_scoring.py first.")
        ...
        if len(filtered) > 0:
            heat_data = filtered[["Latitude", "Longitude", "Severity"]].values.tolist()
            HeatMap(
                heat_data, min_opacity=0.4, radius=35, blur=25, max_zoom=6,
        ...
        for _, row in risk.iterrows():
            ...
            cnt        = len(city_f)
            dom        = city_f["Crime_Description"].value_counts().index[0] if cnt > 0 else "N/A"
            violent    = round(city_f["Is_Violent"].mean() * 100, 1) if cnt > 0 else 0
            night      = round((city_f["Time_Bucket"] == "Night").sum() / cnt * 100, 1) if cnt > 0 else 0

            popup_html = f"""
            <div style="font-family:Arial; font-size:13px; width:230px">
                <b style="font-size:15px">{row['City']}</b>
                <hr style="margin:4px 0">
                <b style="color:{color}">Risk: {row['Risk_Index']:.1f} / 100 — {row['Risk_Category']}</b><br>
        ...
        <div style="position:fixed; bottom:30px; left:30px; z-index:1000;
                    background:black; padding:10px 14px; border-radius:8px;
                    border:2px solid #ccc; font-family:Arial; font-size:12px; color:white">
            <b>Risk Index</b><br>
        ...
        render_folium(india_map._repr_html_(), height=560)
...
def render_risk_tab(risk, filtered):
    ...
    with col1:
        ...
        display["Filtered_Crimes"] = display["Filtered_Crimes"].fillna(0).astype(int)
        display_cols = ["City", "Risk_Index", "Risk_Category", "Crime_Count", "Filtered_Crimes", "Total_Severity"]
    ...
    with col2:
        ...
        colors     = [RISK_COLORS.get(c, "gray") for c in cat_counts.index]
        ...
        ax2.hist(risk["Risk_Index"], bins=10, color="#ef5350", edgecolor="white", alpha=0.8)
...
def render_analytics_tab(filtered):
    ...
    with col1:
        ...
        ax.plot(monthly["Month_Name"], monthly["Count"], marker="o", color="#ef5350", linewidth=2.5, markersize=6)
        ax.fill_between(monthly["Month_Name"], monthly["Count"], alpha=0.15, color="#ef5350")
        for i, row in monthly.iterrows():
            ax.text(i, row["Count"] + 20, str(row["Count"]), ha="center", fontsize=7, color="#aaa")
    ...
    with col2:
        ...
        bucket_data = filtered["Time_Bucket"].value_counts().reindex(bucket_order, fill_value=0)
        colors = ["#f6d860", "#f0a500", "#c0392b", "#1a237e"]
        fig, ax = plt.subplots(figsize=(6, 3.5))
        bars = ax.bar(bucket_order, bucket_data.values, color=colors, edgecolor="none", width=0.6)
        for bar, v in zip(bars, bucket_data.values):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 30, f"{v:,}", ha="center", fontsize=9, color="white")
    ...
    with col3:
        ...
        fig, ax = plt.subplots(figsize=(6, 3.5))
        colors_bar = plt.cm.RdYlGn_r(np.linspace(0.1, 0.9, len(top_cities)))
        bars = ax.barh(top_cities.index, top_cities.values, color=colors_bar, edgecolor="none")
        for bar, v in zip(bars, top_cities.values):
            ax.text(bar.get_width() + 10, bar.get_y() + bar.get_height() / 2, f"{v:,}", va="center", fontsize=8, color="white")
    ...
    with col4:
        ...
        fig, ax    = plt.subplots(figsize=(6, 3.5))
        colors_bar = plt.cm.Oranges(np.linspace(0.4, 0.9, len(top_crimes)))
        bars       = ax.barh(top_crimes.index, top_crimes.values, color=colors_bar, edgecolor="none")
        for bar, v in zip(bars, top_crimes.values):
            ax.text(bar.get_width() + 5, bar.get_y() + bar.get_height() / 2, f"{v:,}", va="center", fontsize=8, color="white")
    ...
    ax.bar(year_counts.index.astype(str), year_counts.values, color="#4fc3f7", edgecolor="none", width=0.5)
...
def render_comparison_tab(df, risk):
    ...
    m1.metric(f"{city_a} Crimes",   f"{len(df_a):,}")
    m1.metric(f"{city_b} Crimes",   f"{len(df_b):,}")
    m2.metric(f"{city_a} Risk",     f"{risk_a['Risk_Index']:.1f}")
    m2.metric(f"{city_b} Risk",     f"{risk_b['Risk_Index']:.1f}")
    m3.metric(f"{city_a} Closure",  f"{round((df_a['Case_Closed']=='Yes').mean()*100,1)}%")
    m3.metric(f"{city_b} Closure",  f"{round((df_b['Case_Closed']=='Yes').mean()*100,1)}%")
    m4.metric(f"{city_a} Violent%", f"{round(df_a['Is_Violent'].mean()*100,1)}%")
    m4.metric(f"{city_b} Violent%", f"{round(df_b['Is_Violent'].mean()*100,1)}%")
    ...
    bucket_order = ["Morning", "Afternoon", "Evening", "Night"]
    counts_a = df_a["Time_Bucket"].value_counts().reindex(bucket_order, fill_value=0)
    counts_b = df_b["Time_Bucket"].value_counts().reindex(bucket_order, fill_value=0)
    x, width = np.arange(len(bucket_order)), 0.35
    fig, ax = plt.subplots(figsize=(10, 4))
    fig.patch.set_facecolor("#0e1117")
    ax.bar(x - width/2, counts_a.values, width, label=city_a, color="#ef5350", edgecolor="none")
    ax.bar(x + width/2, counts_b.values, width, label=city_b, color="#4fc3f7", edgecolor="none")
    ax.set_xticks(x)
...
def render_victim_tab(filtered):
    ...
    with col2:
        ...
        fig, ax = plt.subplots(figsize=(5, 4))
        colors = plt.cm.Blues(np.linspace(0.4, 0.9, len(weapon_counts)))
        ax.barh(weapon_counts.index, weapon_counts.values, color=colors, edgecolor="none")
        for i, v in enumerate(weapon_counts.values):
            ax.text(v + 5, i, f"{v:,}", va="center", fontsize=9, color="white")
        ax.set_xlabel("Count", color="white")
        ax.invert_yaxis()
        ax.set_facecolor("#0e1117")
        fig.patch.set_facecolor("#0e1117")
    ...
    with col3:
        ...
        ax.bar(gender_counts.index, gender_counts.values,
               color=["#2196F3","#E91E63","#9C27B0"], edgecolor="none", width=0.5)
        for i, v in enumerate(gender_counts.values):
            ax.text(i, v + 30, f"{v:,}", ha="center", fontsize=9, color="white")
    ...
    with col4:
        ...
        day_counts = filtered["Day_Name"].value_counts().reindex(day_order, fill_value=0)
        colors = ["#c0392b" if d in ["Saturday","Sunday"] else "#4fc3f7" for d in day_order]
        fig, ax = plt.subplots(figsize=(5, 3))
        fig.patch.set_facecolor("#0e1117")
        ax.bar(day_order, day_counts.values, color=colors, edgecolor="none", width=0.6)
        ax.set_ylabel("Count", color="white")
        ax.annotate("Red = Weekend", xy=(0.7, 0.92), xycoords="axes fraction", fontsize=8, color="#c0392b")
...
def render_season_tab(filtered):
    ...
    with col1:
        ...
        season_counts = filtered["Season"].value_counts().reindex(season_order, fill_value=0)
        fig, ax = plt.subplots(figsize=(5, 3.5))
        fig.patch.set_facecolor("#0e1117")
        bars = ax.bar(season_order, season_counts.values,
                      color=[season_colors[s] for s in season_order], edgecolor="none", width=0.6)
        for bar, v in zip(bars, season_counts.values):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 20, f"{v:,}", ha="center", fontsize=9, color="white")
        ax.set_ylabel("Crimes", color="white")
        ax.set_facecolor("#0e1117")
        ax.tick_params(colors="white")
        ax.grid(axis="y", linestyle="--", alpha=0.3)
    ...
    with col2:
        ...
        fig, ax = plt.subplots(figsize=(5, 3.5))
        fig.patch.set_facecolor("#0e1117")
        ax.fill_between(hour_counts.index, hour_counts.values, alpha=0.4, color="#ef5350")
        ax.plot(hour_counts.index, hour_counts.values, color="#ef5350", linewidth=2)
    ...
    day_order = ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"]
    pivot = filtered.groupby(["Day_Name", "Hour"]).size().unstack(fill_value=0)
    ...
    ax.set_xticklabels([f"{h}:00" for h in range(24)], rotation=45, fontsize=7, color="white")
    ax.set_xlabel("Hour of Day", color="white")
...
def main():
    st.markdown("""
    <style>
    .stMetric { background-color: #1e2130; border-radius: 10px; padding: 14px; border: 1px solid #2d3250; }
    [data-testid="stSidebar"] { background-color: #12141f; }
    .stTabs [data-baseweb="tab-list"] { gap: 8px; }
    .stTabs [data-baseweb="tab"] { background-color: #1e2130; border-radius: 6px; padding: 8px 16px; color: #aaa; }
    .stTabs [aria-selected="true"] { background-color: #ef5350 !important; color: white !important; }
    h3 { color: #4fc3f7; }
    </style>
    """, unsafe_allow_html=True)

    st.markdown("""
    <h1 style='color:#ef5350; margin:0; font-size:2rem'>CriPri: Crime Intelligence Platform prototype</h1>
    ...
    df, risk = load_data()
    (city, crime, bucket, season, risk_cat, year_range) = render_sidebar(df, risk)
    filtered = apply_filters(df, city, crime, bucket, season, risk_cat, year_range)
    st.caption(
        f"Showing **{len(filtered):,}** of **{len(df):,}** records  ,  "
        f"City: {city}  ,  Crime: {crime}  ,  Time: {bucket}  ,  "
        f"Season: {season}  ,  Risk: {risk_cat}  ,  Years: {year_range[0]} to {year_range[1]}"
    )
    render_kpis(filtered, df)

    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        "Maps", "Risk Zones", "Analytics","City Compare", "Victim & Weapon", "Season & Time", "Export Data"
    ])
